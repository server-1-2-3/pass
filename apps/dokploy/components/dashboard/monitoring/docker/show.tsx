	...
	return (
		<div>
			<Card className="bg-background">
				<CardHeader>
					<CardTitle className="text-xl">مانیتورینگ</CardTitle>
					<CardDescription>
						ریزورس استفاده شده از سرور خود را در این قسمت ببینید..
					</CardDescription>
				</CardHeader>
						...
						<div className=" flex-row gap-8 grid md:grid-cols-2 w-full">
							<div className="flex flex-col gap-2  w-full ">
								...
								<span className="text-sm text-muted-foreground">
									استفاده شده: {currentData.cpu.value.toFixed(2)}%
								</span>
							...
							<div className="flex flex-col gap-2  w-full ">
								...
								<span className="text-sm text-muted-foreground">
									{`استفاده شده:  ${(currentData.memory.value.used / 1024 ** 3).toFixed(2)} GB / از: ${(currentData.memory.value.total / 1024 ** 3).toFixed(2)} GB`}
								</span>
								<Progress
							...
							{appName === "dokploy" && (
								<div className="flex flex-col gap-2  w-full ">
									...
									<span className="text-sm text-muted-foreground">
										{`استفاده شده:  ${currentData.disk.value.diskUsage} GB / از: ${currentData.disk.value.diskTotal} GB`}
									</span>
									<Progress
							...
							<div className="flex flex-col gap-2  w-full ">
								<span className="text-base font-medium">نتورک</span>
								<span className="text-sm text-muted-foreground">
									{`ورودی MB: ${currentData.network.value.inputMb.toFixed(
										2,
									)} MB / خروجی MB: ${currentData.network.value.outputMb.toFixed(
										2,
									)} MB`}
								</span>
								<DockerNetworkChart
