	...
	if (active && payload && payload.length && payload[0]) {
		return (
			<div className="custom-tooltip bg-background p-2 shadow-lg rounded-md text-primary border">
				<p>{`تاریخ: ${format(new Date(payload[0].payload.time), "PPpp")}`}</p>
				<p>{`استفاده شده: ${payload[0].payload.usedGb} GB`}</p>
				<p>{`باقی مانده: ${payload[0].payload.freeGb} GB`}</p>
				<p>{`کل فضا: ${payload[0].payload.totalGb} GB`}</p>
			</div>
		);
	}
