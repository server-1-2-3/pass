		...
		try {
			...
			if (auth?.confirmationToken && IS_CLOUD) {
				...
				throw new TRPCError({
					code: "BAD_REQUEST",
					message:
						"Auth Login Error, User failed to access, Not Confirmed.",
				});
			}
...
export const sendVerificationEmail = async (authId: string) => {
	...
	await sendEmailNotification(
		...
		"ایمیل خود را تائید کنید | سرویس پس یک، دو، سه",
		`
		خیلی خیلی خوش آمدید!
		روی لینک زیر کلیک کنید تا حساب کاربری شما تائید شود:
		<a href="${WEBSITE_URL}/confirm-email?token=${result?.confirmationToken}">
			کلیک کنید برای تائید
		</a>
	`,
	);
